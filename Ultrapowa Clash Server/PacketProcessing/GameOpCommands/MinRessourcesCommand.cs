        ...
        public override void Execute(Level level)
        {
            if (level.GetAccountPrivileges() >= GetRequiredAccountPrivileges())
            {
                var p = level.GetPlayerAvatar();
                 ...
                 //, 999999999);
                p.SetResourceCount(ObjectManager.DataTables.GetResourceByName("Gold");
                p.SetResourceCount(ObjectManager.DataTables.GetResourceByName("Elixir");
                p.SetResourceCount(ObjectManager.DataTables.GetResourceByName("DarkElixir");
                p.SetDiamonds(5000);
